...
public class ElasticsearchSinkConnectorConfig extends AbstractConfig {
	...
	private static final String CONNECTION_CDCBROKERS_DOC =
			"The comma-separated list of one or more kafka broker URLs, such as ``http://kafkahost1:9092,"
					+ "http://kafkahost2:9092`` or ``http://kafkahost3:9092``. HTTPS is used for all connections "
	...
	private static final String CONNECTION_CDCTOPIC_DOC =
			"Kafka topic to produce CDC records.";
	...
	protected static ConfigDef baseConfigDef() {
		...
		addCdcConfigs(configDef);
		return configDef;
	}

	private static void addCdcConfigs(ConfigDef configDef) {
		int order=0;
		configDef
		.define(
				CONNECTION_CDCBROKERS_CONFIG,
				Type.LIST,
				CONNECTION_CDCBROKERS_DEFAULT,
				new UrlListValidator(),
				Importance.LOW,
				CONNECTION_CDCBROKERS_DOC,
				CDC_GROUP,
				++order,
				Width.LONG,
				CONNECTION_CDCBROKERS_DISPLAY
				).define(
						CONNECTION_CDCTOPIC_CONFIG,
						Type.STRING,
						CONNECTION_CDCTOPIC_DEFAULT,
						Importance.LOW,
						CONNECTION_CDCTOPIC_DOC,
						CDC_GROUP,
						++order,
						Width.SHORT,
	...
	public Set<String> cdcBrokers() {
		...
		return getList(CONNECTION_CDCBROKERS_CONFIG)
				.stream().map(s -> s.endsWith("/") ? s.substring(0, s.length() - 1) : s)
				.collect(Collectors.toCollection(HashSet::new));
	}
